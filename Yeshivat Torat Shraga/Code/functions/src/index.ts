...
import { log, getURLFor, getRabbiFor, strippedFilename, supplyDefaultParameters } from './helpers';
...
exports.loadRebbeim = functions.https.onCall(async (data, context): Promise<LoadData> => {
	// === APP CHECK ===
	// if (context.app == undefined) {
	//   throw new functions.https.HttpsError(
	//     'failed-precondition',
	...
	// To finish it off, we use await to wait for the Promise returned by Promise.all()
	// to resolve.
	let rebbeimDocs: (RebbeimDocument | null)[] = await Promise.all(
		docs.map(async (doc) => {
			// Get the document data
	...
	return {
		...
		content: rebbeimDocs.filter((doc) => doc != null),
	};
});

exports.loadContent = functions.https.onCall(async (data, context): Promise<LoadData> => {
	// === APP CHECK ===
	// if (context.app == undefined) {
	// 	throw new functions.https.HttpsError(
	// 		'failed-precondition',
	...
	// To finish it off, we use await to wait for the Promise returned by Promise.all()
	// to resolve.
	let contentDocs: (ContentDocument | null)[] = await Promise.all(
		docs.map(async (doc) => {
			// Get the document data
			const data = doc.data() as ContentFirebaseDocument;
			try {
				let source_path = await getURLFor(`${data.source_path}`);
				let author = await getRabbiFor(doc.id, queryOptions.includeAllAuthorData);
				return {
					id: doc.id,
	...
	return {
		...
		content: contentDocs.filter((doc) => doc != null),
	};
});

exports.generateHLSStream = functions.storage
	.bucket()
	.object()
		...
		try {
			await childProcessPromise.spawn(
				...
				[
					...
					`${outputDir}/${foldername}-%03d.ts`,
					`${outputDir}/${foldername}.m3u8`,
				],
				{
					stdio: 'inherit',
				}
...
exports.searchFirestore = functions.https.onCall(async (callData, context): Promise<any> => {
